...
public class MainActivity extends AppCompatActivity {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                case 1: // 注册
                    break;
                case 2: // 登录
                    UsrLogin1((Map<String, String>) msg.obj);
                    break;
            }
        }
    };
        ...
        InfoDiaglog.setPositiveButton("下一步",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface judge_dialog, int which) {
                        ...
                        newCard.RealTalkTime = 0;
                        newCard.RealSMSCount = 0;
                        newCard.RealSMSCount = 0;

                        card.put(newCard.CardNumber, newCard);
                        Toast toast = Toast.makeText(getApplicationContext(), "注册中，请稍等。", Toast.LENGTH_SHORT);
                        toast.show();

                        String dbInfo = "INSERT INTO MobileCardMYSQL VALUES (";
                        dbInfo += "\"" + newCard.CardNumber   + "\", " +
                                  "\"" + newCard.UserName     + "\", " +
                                  "\"" + newCard.PassWord     + "\", " +
                                  "\"" + newCard.SerPackage.GetPackageName() + "\", " +
                                         newCard.ConsumAmount + ", "   +
                                         newCard.Money        + ", 0, 0, 0);";
                        final String final_dbInfo = dbInfo;
                        new Thread(new Runnable() {
                            public void run() {
                                DBConnection db = new DBConnection();
                                db.linkMysql(final_dbInfo, OPTION.EM_REGISTER, null);
                            }
                        }).start();
    ...
    public void UsrLogin(View v){
        ...
        InfoDiaglog.setPositiveButton("下一步",
                new DialogInterface.OnClickListener() {
                    ...
                    if(!card.containsKey(GetNumber)) //合法性判断，是否已经注册，先在cache中查找
                    {
                        AlertDialog.Builder result = new AlertDialog.Builder(MainActivity.this);
                        result.setTitle("错误");
                        if(GetNumber.isEmpty()) {
                            ...
                            // 远端数据库查一下有没有
                            final String dbInfo = "SELECT PassWord from MobileCardMYSQL WHERE CardNumber = \"" + GetNumber + "\";";
                            new Thread(new Runnable() {
                                public void run() {
                                    DBConnection db = new DBConnection();
                                    Map other_info = new HashMap();
                                    other_info.put("Number", GetNumber);
                                    other_info.put("UsrPassword", GetPassword);
                                    ReturnData returnData = db.linkMysql(dbInfo, OPTION.EM_LOGIN, other_info);
                                    Message msg = new Message();
                                    msg.what = returnData.id;
    ...
    public void UsrLogin1(final Map<String, String> SearchResult){
        System.out.println("enterUsrlogin1");
        AlertDialog.Builder result = new AlertDialog.Builder(MainActivity.this);
        if(SearchResult.get("CorrectPassword").equals("null")) {
            System.out.println("enterUsrlogin1-1");
            result.setTitle("错误");
        ...
        {
            if (!SearchResult.get("CorrectPassword").equals(SearchResult.get("UsrPassword"))) {
                result.setTitle("错误");
                result.setMessage("登录失败：密码错误！");
                result.show();
            } else {
                ...
                result.setPositiveButton("下一步",
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface judge_dialog, int which) {
                                Intent MyIntent = new Intent(MainActivity.this,SecondActivity.class);
                                MyIntent.putExtra("number", SearchResult.get("Number")); //参数为用户手机号
                                startActivityForResult(MyIntent,1); // 启动Activity
                            }
                        });
                result.show();
